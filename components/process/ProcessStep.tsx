    ...
    >
      ...
      <div>
        <div style={{
          ...
          letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--gold)', marginBottom: '16px',
        }}>
          {step.label}
        </div>
        <h2 style={{
      ...
      <div>
        <p style={{ fontFamily: 'var(--font-body)', fontSize: '14px', fontWeight: 300, color: 'var(--muted)', lineHeight: 1.85, marginBottom: '20px' }}>
          {step.description}
        </p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {step.includes.map((item) => (
            <div key={item} style={{ display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
              <span style={{ width: '3px', height: '3px', background: 'var(--gold-dim)', borderRadius: '50%', flexShrink: 0, marginTop: '7px' }} />
              <span style={{ fontFamily: 'var(--font-body)', fontSize: '12px', fontWeight: 300, color: 'var(--muted)', lineHeight: 1.7 }}>
                {item}
              </span>
            </div>
          ))}
